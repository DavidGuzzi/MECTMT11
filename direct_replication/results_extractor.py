...
def extract_state_space_matrices(oc: Oct2Py) -> Dict[str, np.ndarray]:
    ...
    Returns:
        Dictionary with keys:
            - 'T': State transition matrix (n_states × n_states)
            - 'R': Shock impact matrix (n_states × n_shocks)
            - 'Z': Measurement matrix (n_obs × n_states)
...
def extract_forecast_results(oc: Oct2Py, horizon: int) -> Dict[str, np.ndarray]:
    ...
    Returns:
        Dictionary with:
            - 'forecasts': Forecast values (n_vars × horizon)
...
def extract_shock_decomposition(oc: Oct2Py) -> pd.DataFrame:
    ...
    try:
        ...
        # shock_decomp is (n_vars × n_shocks × n_periods)
